...
const CourseManagement = () => {
  ...
  const validate = () => {
    const errs = {};
    if (!form.name.trim()) errs.name = "Vui lòng nhập tên khóa học";
    if (!form.level) errs.level = "Vui lòng chọn cấp độ";
    if (!form.description.trim()) errs.description = "Vui lòng nhập mô tả";
    if (!form.price) errs.price = "Vui lòng nhập học phí";
    else if (form.price < 2000000)
      errs.price = "Học phí tối thiểu là 2 triệu VND";
    if (!form.total_sessions) errs.total_sessions = "Vui lòng nhập số buổi học";
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => ({
      ...prev,
      [name]: files ? files[0] : name === "active" ? value === "true" : value,
    }));
