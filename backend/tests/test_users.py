...
class TestUserModule:
    ...
    def test_password_validation_fails(self, pwd):
    ...
    def test_get_current_user_when_authenticated_returns_user_info(self, api_client, active_user):
    ...
    def test_update_current_user_when_authenticated_updates_profile_successfully(self, api_client, active_user):
    ...
    def test_delete_current_user_when_authenticated_soft_deletes_user(self, api_client, active_user):
    ...
    def test_update_password_when_authenticated_updates_password_successfully(self, api_client, active_user):
    ...
    def test_generate_auth_token_util(self, active_user):
    ...
    def test_get_current_user_when_unauthenticated_returns_401(self, api_client):
        """ Hàm này test nếu user chưa đăng nhập thì không thể xem thông tin cá nhân """
        url = reverse('user-current-user')
        response = api_client.get(url)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_create_user_when_admin_assigns_teacher_group(self, api_client, admin_user, teacher_group):
    ...
    def test_update_avatar_when_authenticated_updates_profile_avatar_successfully(self, api_client, active_user):
    ...
    def test_get_payments_returns_only_user_payments(self, api_client, active_user, classroom):
    ...
    def test_social_login_when_missing_data_returns_400_error(self, api_client):
    ...
    def test_social_login_when_provider_invalid_returns_400_error(self, api_client):
    ...
    def test_delete_user_when_admin_soft_deletes_user_successfully(self, api_client, admin_user):
        """ Test logic admin xóa tài khoản người dùng (xóa mềm)"""
        api_client.force_authenticate(user=admin_user)
        
        target = baker.make('users.User', is_active=True)
        url = reverse('user-detail', kwargs={'pk': target.id})
    ...
    def test_delete_user_when_not_admin_returns_403(self, api_client, active_user):
    ...
    def test_social_login_with_google_when_valid_token_creates_user_and_returns_access_token(self, api_client, mocker):
    ...
    def test_social_login_google_invalid_token_response(self, api_client, mocker):
    ...
    def test_get_user_list_when_admin_returns_correct_users(self, api_client, admin_user):
    ...
    def test_user_list_when_unauthenticated_returns_401(self, api_client):
        response = api_client.get(reverse('user-list'))
