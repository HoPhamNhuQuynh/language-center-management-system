...
from classes.models import Session, TeachingAssignment
...
class TestClassesModels:
    ...
    def test_UTL_002_classroom_is_auto_active_when_enrollment_reaches_threshold(
        self, classroom
    ):
    ...
    def test_UTL_005_session_str_returns_class_and_date(self, classroom):
        ...
        session = baker.make(
            "classes.Session", schedule=schedule, date=datetime.date(2026, 4, 23)
        )

        expected_str = f"class_{classroom.id}_at:_{session.date}"
        assert str(session) == expected_str

    def test_UTL_006_teaching_assignment_str_returns_teacher_and_classroom(
        self, classroom, active_teacher
    ):
        """Hàm này unit test str của model phân công giảng dạy - teachingassignment"""
        assignment = baker.make(
            "classes.TeachingAssignment", teacher=active_teacher, classroom=classroom
        )

        expected_str = f"{active_teacher.username}-{classroom.name}"
        assert str(assignment) == expected_str

    def test_UTL_007_classroom_does_not_auto_activate_when_less_than_10_students(
        self, classroom
    ):
...
class TestClassRoomSerializer:
    ...
    def test_UTL_008_classroom_date_validation_bva(
        self, classroom, start, end, should_fail
    ):
    ...
    def test_UTL_009_classroom_serializer_creates_main_teacher_assignment_when_main_teacher_provided(
        self, classroom
    ):
        ...
        assert classroom.teachingassignment_set.filter(
            teacher=teacher, is_main=True
        ).exists()

    def test_UTL_010_classroom_serializer_updates_main_teacher_and_switches_assignment(
        self, classroom
    ):
        ...
        assert (
            classroom.teachingassignment_set.get(teacher=old_teacher).is_main is False
        )
    ...
    def test_UTL_012_classroom_cannot_be_deleted_when_has_enrollments(
        self, api_client, classroom, admin_user
    ):
    ...
    def test_UTL_014_switch_main_teacher_removes_old_flag(self, classroom):
        ...
        baker.make(
            "classes.TeachingAssignment", classroom=classroom, teacher=old, is_main=True
        )

        serializer = ClassRoomSerializer(
            instance=classroom, data={"main_teacher_id": new.id}, partial=True
        )
    ...
    def test_UTL_037_classroom_validate_session_count_mismatch(self, classroom):
        ...
        serializer = ClassRoomSerializer(data=data)
        assert not serializer.is_valid()
        assert "nhưng khóa học yêu cầu 20 buổi" in str(
    ...
    def test_UTL_038_classroom_update_prevented_when_attendance_exists(self, classroom):
        ...
        assert "Không thể thay đổi lịch học vì đã có dữ liệu điểm danh." in str(
            exc.value
        )
...
class TestScheduleSerializer:
    def test_UTL_015_schedule_serializer_raises_error_when_room_has_overlapping_time(
        self,
    ):
    ...
    @pytest.mark.parametrize(
        "start, end, should_fail",
        [
            ("15:00:00", "17:00:00", True),
            ("14:30:00", "15:30:00", True),
            ("17:00:00", "18:00:00", False),
            ("08:00:00", "10:00:00", False),
        ]
    )
    def test_UTL_016_session_serializer_raises_error_when_teacher_has_overlapping_session(
        self,
        start,
        end,
        should_fail,
    ):
        """Kiểm tra trùng lịch giáo viên khi gán cho buổi học"""
        ...
        classroom_1 = baker.make(
            "classes.ClassRoom",
            course=course,
        )

        classroom_2 = baker.make(
            "classes.ClassRoom",
            course=course,
        )

        room_1 = baker.make("classes.Room")
        room_2 = baker.make("classes.Room")
        ...
        if should_fail:
            assert not serializer.is_valid()
            assert "Giáo viên đã có lịch dạy bị trùng" in str(
                serializer.errors
            )
        else:
            assert serializer.is_valid(), serializer.errors

    def test_UTL_017_session_serializer_auto_create_teaching_assignment(
        self, classroom
    ):
        """Kiểm tra khi gán giáo viên cho buổi học thì tự tạo phân công"""
...
class TestOtherSerializers:
    ...
    def test_UTL_020_session_serializer_includes_extra_fields_for_admin_user(
        self, admin_user
    ):
    ...
    def test_UTL_021_classroom_get_sessions_returns_200(
    ...
    def test_UTL_022_classroom_get_scores_returns_200_for_admin(
        self, api_client, classroom, admin_user
    ):
    ...
    def test_UTL_024_delete_classroom_protected_error_returns_400_by_admin(
        self, api_client, classroom, admin_user
    ):
    ...
    def test_UTL_027_queryset_student_filters_capacity(
        self, api_client, active_user, classroom
    ):
        api_client.force_authenticate(user=active_user)
    ...
    def test_UTL_028_classroom_create_permission_denied_for_student(
    ...
    def test_UTL_029_retrieve_uses_detail_serializer(
        self, api_client, admin_user, classroom
    ):
        api_client.force_authenticate(user=admin_user)

        response = api_client.get(
            reverse("classroom-detail", kwargs={"pk": classroom.id})
        )

        assert "main_teacher" in response.data or "course" in response.data

    def test_UTL_030_teacher_can_access_scores(
        self, api_client, active_teacher, classroom
    ):
    ...
    def test_UTL_037_session_serializer_planned_sessions_exceeded(self, classroom):
        ...
        serializer = SessionSerializer(data=data)
        assert not serializer.is_valid()
        assert "Đã đủ 1 buổi theo kế hoạch" in str(
            serializer.errors["non_field_errors"]
        )
...
class TestClassesViews:
    ...
    def test_UTL_043_session_perform_destroy_protected_error(
